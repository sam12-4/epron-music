<?php
    $tg_blog_display_related = get_theme_mod('tg_blog_display_related', false);
...
if ($tags) {
 ...
 ?>
	...
	<h3><?php echo esc_html_e('Related Articles', 'musico' ); ?></h3><br class="clear"/>
    <?php
       ...
       $image_thumb = '';
		...
		if(has_post_thumbnail(get_the_ID(), 'musico-blog'))
		{
		    $image_id = get_post_thumbnail_id(get_the_ID());
		    $image_thumb = wp_get_attachment_image_src($image_id, 'musico-blog');
		}
    ?>
			...
			<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<div class="post_wrapper grid_layout">
					<?php
					    ...
					    {
					     $small_image_url = wp_get_attachment_image_src($image_id, 'musico-blog', true);
					?>
					    <div class="post_img static">
						    ...
						    <div class="post_img_hover classic <?php if(!empty($tg_enable_lazy_loading)) { ?>lazy<?php } ?>">
						     	<?php 
							     	$blog_featured_img_url = get_the_post_thumbnail_url($post, 'musico-blog'); 
							     	$blog_featured_img_data = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), "musico-blog" );
							     	$blog_featured_img_alt = get_post_meta(get_post_thumbnail_id($post->ID), '_wp_attachment_image_alt', true);
							     	$return_attr = musico_get_lazy_img_attr();
							     	
							     	if(!empty($blog_featured_img_url))
							     	{
							     ?>
							     <img <?php echo musico_get_blank_img_attr(); ?> <?php echo esc_attr($return_attr['source']); ?>="<?php echo esc_url($blog_featured_img_url); ?>" class="<?php echo esc_attr($return_attr['class']); ?>" alt="<?php echo esc_attr($blog_featured_img_alt); ?>"/>
							     <?php
								     }
								?>
						     	<?php echo musico_get_post_format_icon(get_the_ID()); ?>
						...
						<div class="post_header_wrapper">
							<div class="post_button_wrapper">
						    	<div class="post_detail">
								    <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo date_i18n(MUSICO_THEMEDATEFORMAT, get_the_time('U')); ?></a>
								</div>
						    </div>
						</div>
						<?php
