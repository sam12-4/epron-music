...
<?php
    ...
    if(MUSICO_THEMEDEMO && isset($_GET['topbar']) && !empty($_GET['topbar']))
	{
	    $tg_topbar = true;
	}
    
    $musico_topbar = musico_get_topbar();
    musico_set_topbar($tg_topbar);
...
<?php
   ...
   if(!empty($pp_page_bg) && basename($pp_page_bg)=='default.png')
	...
	if(class_exists('Woocommerce') && musico_is_woocommerce_page())
	{
	...
	if($musico_homepage_style == 'fullscreen')
	{
...
<div class="top_bar <?php if(!empty($page_menu_transparent)) { ?>hasbg<?php } ?>">
		...
		<div id="logo_right_button">
			...
			<div class="header_client_wrapper">
			    <a class="client_login_link" href="javascript:;" title="<?php esc_attr_e('Member Login', 'musico' ); ?>"><span class="ti-lock"></span><?php esc_html_e('Member Login', 'musico' ); ?></a>
			</div>
			<?php
				}
			...
			?>
			<div class="header_client_wrapper">
			    <span class="ti-user"></span><a href="<?php echo esc_url($user_home_url); ?>"><?php echo esc_html($current_user->display_name); ?></a>&nbsp;(<a class="client_logout_link" href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php esc_attr_e('Logout', 'musico' ); ?>"><?php esc_html_e('Logout', 'musico' ); ?></a>)
			</div>
			<?php
				}
			?>
		    ...
		    <?php
			    ...
			    $woocommerce = musico_get_woocommerce();
			    $cart_url = wc_get_cart_url();
			    $cart_count = $woocommerce->cart->cart_contents_count;
			?>
			<div class="header_cart_wrapper">
			    <div class="cart_count"><?php echo esc_html($cart_count); ?></div>
			    <a class="tooltip" href="<?php echo esc_url($cart_url); ?>" title="<?php esc_attr_e('View Cart', 'musico' ); ?>"><span class="ti-shopping-cart"></span></a>
			</div>
			<?php
			}
			?>
    	...
    	<?php
    	    //get custom logo
    	    $tg_retina_logo = get_theme_mod('tg_retina_logo', musico_get_demo_logo('tg_retina_logo'));
    	    ...
    	    {	
		    	...
		    	else if($tg_retina_logo == musico_get_demo_logo('tg_retina_logo'))
		    	{
			    	$image_width = 0;
			    	$image_height = 0;
		    	}
    		...
    		<div class="logo_align">
	    	    <a id="custom_logo" class="logo_wrapper <?php if(!empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
					...
					<?php
						...
						else if($image_width == 1 && $image_height == 1 && $tg_retina_logo != musico_get_demo_logo('tg_retina_logo'))
						{
					?>
	    	    	...
	    	    	<?php 
						...
						{
					?>
	    	    	<img src="<?php echo esc_url($tg_retina_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="101" height="19"/>
	    	    	<?php 
		    	    	}
		    	    ?>
	    	    </a>
    	...
    	<?php
    	    ...
    	    $tg_retina_transparent_logo = get_theme_mod('tg_retina_transparent_logo', musico_get_demo_logo('tg_retina_transparent_logo'));

    	    if(!empty($tg_retina_transparent_logo))
    	    {
    	    	//Get image width and height
		    	$image_id = musico_get_image_id($tg_retina_transparent_logo);
			    ...
			    else if($tg_retina_transparent_logo == musico_get_demo_logo('tg_retina_transparent_logo'))
		    	{
			    	$image_width = 0;
			    	$image_height = 0;
		    	}
    		...
    		<div class="logo_align">
	    	    <a id="custom_logo_transparent" class="logo_wrapper <?php if(empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
	    	    	...
	    	    	<?php 
						...
						{
					?>
	    	    	<img src="<?php echo esc_url($tg_retina_transparent_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="101" height="19"/>
	    	    	<?php 
		    	    	}
		    	    ?>
	    	    </a>
