...
if(MUSICO_THEMEDEMO && isset($_GET['sidebar']))
{
...
?>
<div id="page_caption" class="<?php if(!empty($pp_page_bg)) { ?>hasbg <?php if(!empty($tg_page_header_bg_parallax)) { ?>parallax<?php } ?> <?php } ?> <?php if(!empty($musico_topbar)) { ?>withtopbar<?php } ?> <?php if(!empty($musico_screen_class)) { echo esc_attr($musico_screen_class); } ?> <?php if(!empty($musico_page_content_class)) { echo esc_attr($musico_page_content_class); } ?>" <?php if(!empty($pp_page_bg)) { ?>style="background-image:url(<?php echo esc_url($pp_page_bg); ?>);"<?php } ?> <?php if($page_header_type == 'Youtube Video' OR $page_header_type == 'Vimeo Video') { ?>data-jarallax-video="<?php echo esc_url($video_url); ?>"<?php } ?>>
